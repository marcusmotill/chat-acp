...
class TestAgentEnv(unittest.IsolatedAsyncioTestCase):
    async def test_agent_env_passing(self):
        ...
        workspace = Workspace(id="1", environment_id="env1", name="ws1", target_path="/tmp")
        ...
        with patch("asyncio.create_subprocess_exec") as mock_exec:
            ...
            with patch.object(AcpStdioAgent, "send_request", new_callable=AsyncMock) as mock_send:
                mock_send.return_value = MagicMock(error=None, result={"sessionId": "test-agent-session"})
